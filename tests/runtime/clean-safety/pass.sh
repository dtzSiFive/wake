...
cleanup() {
  # Kill the entire process group (wake + fuse-waked + wakebox + children)
  if [ -n "${WAKE_PID:-}" ]; then
...
# Use setsid so wake becomes process group leader for clean shutdown
setsid "${WAKE}" -x 'longRunningBuild Unit' &
WAKE_PID=$!
...
# Kill the build process group (simulates crash)
kill -9 -- -"$WAKE_PID"
wait "$WAKE_PID" 2>/dev/null || true
unset WAKE_PID
