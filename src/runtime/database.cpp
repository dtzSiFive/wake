...
std::string Database::open(bool wait, bool memory, bool tty, bool readonly) {
  ...
  const char *sql_update_file = "update files set modified=? where hash=? and type=? and mode=? and path=?";
...
template <typename F>
bool foreach_pathinfo(std::string_view s, F &&f) {
  static_assert(std::is_invocable_v<F &, PathInfo>,
                "Callback must be invocable with PathInfo");
