...
static std::vector<Migration> get_migrations() {
  return {
      ...
      // Version 10 -> 11: Per-job hashes
      // - Remove stale column from jobs table
      // - Change files index from unique(path) to unique(path, hash)
      ...
      {10, 11,
       [](sqlite3* db) -> bool {
         ...
         // Drop the old unique index on path (if it exists)
         // SQLite index names may vary, so we recreate the table to be safe
         ...
         if (!exec_sql(db, "INSERT INTO files_new SELECT * FROM files;")) return false;
         ...
         if (!exec_sql(db, "CREATE UNIQUE INDEX file_path_hash ON files(path, hash);"))
           return false;
         if (!exec_sql(db, "CREATE INDEX filenames ON files(path);")) return false;

         return true;
       },
       "Per-job hashes: remove stale column, unique index on (path, hash)"},
